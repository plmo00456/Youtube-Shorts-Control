...
window.onload = function() {
  ...
  const dragFn = (layer) => {
    ...
    layer.addEventListener("mousedown", dragStart, false);
    ...
    function drag(e) {
      if (active) {
        ...
        setTranslate(currentX, currentY, layer);
      }
    }
    ...
    if(video
      ...
      div2.innerHTML = `
        ...
        <div id="custom-help" class="help">
          ...
          <div class="row title">
            <span>${chrome.i18n.getMessage('msg_video_help_title_playback')}</span>
          </div>
          <div class="row">
            <span>${chrome.i18n.getMessage('msg_video_help_pause')}</span>
            <span>k</span>
          </div>
          <div class="row">
            <span>${chrome.i18n.getMessage('msg_video_help_move_rewind')}</span>
            <span>← &nbsp;${chrome.i18n.getMessage('msg_video_help_shortcut_or')} &nbsp;j</span>
          </div>
           <div class="row">
            <span>${chrome.i18n.getMessage('msg_video_help_move_forward')}</span>
            <span>→ &nbsp;${chrome.i18n.getMessage('msg_video_help_shortcut_or')} &nbsp;l </span>
          </div>
          <div class="row">
          ...
          <div class="row">
            ...
            <span>- &nbsp;${chrome.i18n.getMessage('msg_video_help_shortcut_or')} &nbsp;[</span>
          </div>
          <div class="row">
          ...
          <div class="row">
            <span>${chrome.i18n.getMessage('msg_video_help_move_point')}</span>
            <span>0 . . 9</span>
          </div>
          <div class="row">
            <span>${chrome.i18n.getMessage('msg_video_help_move_previous')}</span>
            <span>↑</span>
          </div>
          <div class="row">
            <span>${chrome.i18n.getMessage('msg_video_help_move_next')}</span>
            <span>↓</span>
          </div>
          <div class="row title" style="margin-top: 20px;">
            <span>${chrome.i18n.getMessage('msg_video_help_title_general')}</span>
          </div>
          <div class="row">
            <span>${chrome.i18n.getMessage('msg_video_help_autoplay')}</span>
            <span>o</span>
          </div>
          <div class="row">
            <span>${chrome.i18n.getMessage('msg_video_help_pip')}</span>
            <span>p</span>
          </div>
          <div class="row">
            <span>${chrome.i18n.getMessage('msg_video_help_mute')}</span>
            <span>m</span>
          </div>
        </div>
      `;
      ...
      dragFn(document.querySelector('#custom-help'));
      ...
      volumeRange.oninput = () => {
        ...
        if(parseInt(volumeRange.value) === 0){
          ...
          if(parseInt(volumeRange.value) <= 60){
            volumeWrap.querySelector('.one').classList.add('show');
          }else {
            volumeWrap.querySelector('.two').classList.add('show');
          }
      ...
      } else if (updateLayerEvent && (chrome.runtime.getManifest().version !== data['version'])) {
        // 업데이트 시 실행
        settingBtn.classList.add('active');
        helpWindow.classList.add('active');
